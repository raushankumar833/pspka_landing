...
import { Alert, Box, Card, Grid, Stack, styled, useTheme } from '@mui/material';
...
import { getBbpsBillerDetails } from 'src/redux/my-slices/bbps';
...
const BBPSPaymentsTab = ({ serviceSubType }: any) => {
  ...
  const { selectedBiller, isLoading, bbpsbillerDetails } = useSelector((state) => state.bbps);

  const MobileSchema = useMemo(() => {
    if (checklength(bbpsbillerDetails)) {
      const sch: any = {
        sc: Yup.object().shape(createDynamicSchema(bbpsbillerDetails)),
      };
      return sch;
    }
  }, [bbpsbillerDetails]);
  ...
  useEffect(() => {
    if (selectedBiller?.biller_id)
      dispatch(getBbpsBillerDetails({ biller_id: selectedBiller.biller_id }));
  ...
  const inputParameters = useMemo(() => {
    if (checklength(bbpsbillerDetails)) {
      ...
      // console.log('start');
      const renderedParam =
        checklength(bbpsbillerDetails) &&
        bbpsbillerDetails.map((item: any, index: any) => {
          if (
            item.inputType.toUpperCase() === 'NUMERIC' ||
            item.inputType.toUpperCase() === 'ALPHANUMERIC'
          ) {
            return (
              <RHFTextField
                key={index}
    ...
    <>
      ...
      <Grid container spacing={3}>
        ...
        >
          ...
          <RechargesCard>
            <PermissionGaurd permission={selectedBiller?.biller_id ? true : false}>
              <OuterIcon bg={selectedBiller?.color || theme.palette.primary.main}>
                <InnerIcon>
                  <img src={selectedBiller?.icon_url} alt={selectedBiller?.biller_id} />
                </InnerIcon>
              </OuterIcon>
            </PermissionGaurd>
            <InnerCard bg={selectedBiller?.color || theme.palette.primary.main}>
              <CardTitle
                title={selectedBiller?.biller_name || 'Select Biller or Category'}
                sx={{
              ...
              <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
                ...
                >
                  ...
                  {selectedBiller?.biller_id ? (
                    ...
                    <img src={serviceSubType?.ref_img} alt="" />
                  )}
                </Stack>
              </FormProvider>
            </InnerCard>
      ...
      <PermissionGaurd permission={selectedBiller?.biller_id && serviceSubType?.banner}>
        <Box sx={{ mt: 2 }}>
          <img src={serviceSubType?.banner} alt="banner" />
        </Box>
      </PermissionGaurd>
      ...
      {openTpin && (
        <TPINDialogue
          apiEnd={Apiendpoints.BBPS_BILL_PAYMENT}
          completeData={data}
...
export default BBPSPaymentsTab;
